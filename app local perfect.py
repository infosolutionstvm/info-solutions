...
# --- 2. ലോഗിൻ സിസ്റ്റം ഇവിടെ ചേർക്കുക ---
def check_password():
    if "authenticated" not in st.session_state:
        st.session_state["authenticated"] = False

    if not st.session_state["authenticated"]:
        st.subheader("🔒 INFO SOLUTIONS - Login")
        pwd = st.text_input("Enter Password", type="password")
        if st.button("Login"):
            if pwd == "passwordilla":  # 👈 നിങ്ങളുടെ പാസ്‌വേഡ് ഇവിടെ നൽകുക
...
if not check_password():
    st.stop()  # പാസ്‌വേഡ് ശരിയല്ലെങ്കിൽ ആപ്പിന്റെ ബാക്കി ഭാഗം ലോഡ് ആകില്ല

# --- 3. പേജ് കോൺഫിഗറേഷൻ (തുടർന്നുള്ള കോഡുകൾ) ---
st.set_page_config(
...
# Helper Function for Detailed WhatsApp Message Generation
...
st.sidebar.divider()  # മെനുവിന് താഴെ ചെറിയൊരു ലൈൻ
if st.sidebar.button("🚪 Logout", key="logout_btn", use_container_width=True, type="secondary"):
    st.session_state["authenticated"] = False
    st.rerun()

# ==========================================
# MAIN PAGES CONTENT (കണ്ടീഷനുകൾ ഇതിന് ശേഷം വരണം)
# ==========================================
...
if menu == "Dashboard & Operations":
    ...
    with right_col:
        ...
        if filtered_rows:
            ...
            tab_update, tab_receipt = st.tabs(["✏️ Update Status", "📄 Print Receipt"])

            with tab_update:
                st.write("")
                u1, u2 = st.columns(2)
                with u1:
                    new_status = st.selectbox("Update Status", ["Pending", "Completed", "Delivered"], index=["Pending", "Completed", "Delivered"].index(selected_data[7]))
    ...
    with tab_new:
        ...
        with q_col2:
            ...
            if items:
                ...
                q_html = generate_quotation_html(generated_no, today_str, q_cust_name, q_phone, q_type, items, subtotal, total_tax, grand_total)
                components.html(q_html, height=800, scrolling=True)

    # --- TAB 2: EDIT / DELETE / REPRINT QUOTATIONS ---
    with tab_edit:
        if all_quotes:
            q_dict = {f"{q[0]} | {q[1]} ({q[3]}) - ₹{q[8]:,.2f}": q for q in all_quotes}
            selected_q_label = st.selectbox("Select Saved Quotation to Edit/View:", list(q_dict.keys()))
            selected_q = q_dict[selected_q_label]

            q_no, cust_name, phone, category, q_date, items_json, subtotal, tax_total, grand_total = selected_q

            # Manage state for editing items
            if "edit_q_no" not in st.session_state or st.session_state["edit_q_no"] != q_no:
                st.session_state["edit_q_no"] = q_no
                st.session_state["edit_items"] = json.loads(items_json)

            categories_list = [
                "Computer & Laptop Sales/Service", "CCTV Surveillance System", 
                "Networking & Wi-Fi Solutions", "Solar Power System Installation", "General Service Enquiry"
            ]
            cat_index = categories_list.index(category) if category in categories_list else 0

            eq_col1, eq_col2 = st.columns([1.1, 1.4], gap="large")

            with eq_col1:
                st.markdown("#### ✏️ Edit Customer Details")
                e_cust_name = st.text_input("Customer Name", value=cust_name, key="eq_name")
                e_phone = st.text_input("Phone Number", value=phone, key="eq_phone")
                e_category = st.selectbox("Category", categories_list, index=cat_index, key="eq_cat")

                st.divider()
                st.markdown("#### 📦 Edit Line Items")

                # Show existing edit items with inline modifier controls
                for idx, item in enumerate(st.session_state["edit_items"]):
                    st.write(f"**Item #{idx+1}: {item['desc']}**")
                    ic1, ic2, ic3, ic4 = st.columns([2, 2, 2, 1])
                    with ic1:
                        new_q = st.number_input("Qty", min_value=1, value=int(item['qty']), key=f"eq_qty_{idx}")
                    with ic2:
                        new_r = st.number_input("Rate (₹)", min_value=0.0, value=float(item['rate']), step=100.0, key=f"eq_rate_{idx}")
                    with ic3:
                        new_t = st.number_input("GST %", min_value=0.0, max_value=28.0, value=float(item['tax_pct']), step=1.0, key=f"eq_tax_{idx}")
                    with ic4:
                        st.write("")
                        st.write("")
                        if st.button("❌", key=f"eq_del_{idx}"):
                            st.session_state["edit_items"].pop(idx)
                            st.rerun()

                    # Calculate total for modified line item
                    tot_base = new_q * new_r
                    tax_val = tot_base * (new_t / 100.0)
                    st.session_state["edit_items"][idx]['qty'] = new_q
                    st.session_state["edit_items"][idx]['rate'] = new_r
                    st.session_state["edit_items"][idx]['tax_pct'] = new_t
                    st.session_state["edit_items"][idx]['tax_amt'] = tax_val
                    st.session_state["edit_items"][idx]['total'] = tot_base + tax_val
                    st.write("---")

                # Option to add extra item to selected quotation
                st.markdown("##### ➕ Add New Item to this Quotation")
                e_selected_master = st.selectbox("Search Master Product", prod_options, key="edit_master_selector")
                
                e_def_name, e_def_rate, e_def_tax = "", 0.0, 18.0
                if e_selected_master != "-- Type / Select Existing Product --":
                    e_def_name = e_selected_master
                    e_def_rate = prod_dict[e_selected_master]["rate"]
                    e_def_tax = prod_dict[e_selected_master]["tax"]

                with st.form("add_edit_item_form", clear_on_submit=True):
                    e_item_name = st.text_input("Item Name", value=e_def_name)
                    e_c1, e_c2, e_c3 = st.columns(3)
                    with e_c1: e_qty = st.number_input("Qty", min_value=1, value=1)
                    with e_c2: e_rate = st.number_input("Price (₹)", min_value=0.0, value=e_def_rate, step=100.0)
                    with e_c3: e_tax = st.number_input("GST %", min_value=0.0, max_value=28.0, value=e_def_tax)
                    
                    if st.form_submit_button("➕ Append Item"):
                        if e_item_name.strip():
                            save_or_update_product(e_item_name, e_rate, e_tax)
                            it_tot = e_qty * e_rate
                            tx_val = it_tot * (e_tax / 100.0)
                            st.session_state["edit_items"].append({
                                "desc": e_item_name, "qty": e_qty, "rate": e_rate,
                                "tax_pct": e_tax, "tax_amt": tx_val, "total": it_tot + tx_val
                            })
                            st.rerun()
                ...
                with btn_u1:
                    if st.button("💾 UPDATE QUOTATION", type="primary", use_container_width=True):
                        e_items = st.session_state["edit_items"]
                        if not e_items:
                            st.error("Cannot save empty quotation! Add at least one item.")
                        else:
                            e_subtotal = sum(i["qty"] * i["rate"] for i in e_items)
                            e_tax_total = sum(i["tax_amt"] for i in e_items)
                            e_grand_total = e_subtotal + e_tax_total
                            ...
                            cursor.execute("""
                                ...
                                WHERE quote_no=?
                            """, (e_cust_name, e_phone, e_category, json.dumps(e_items), e_subtotal, e_tax_total, e_grand_total, q_no))
                            conn.commit()
                            conn.close()
                            st.toast("Quotation updated successfully!", icon="✅")
                            st.rerun()
            ...
            with eq_col2:
                current_e_items = st.session_state["edit_items"]
                e_subtotal = sum(i["qty"] * i["rate"] for i in current_e_items)
                e_tax_total = sum(i["tax_amt"] for i in current_e_items)
                e_grand_total = e_subtotal + e_tax_total

                if e_phone.strip():
                    wa_eq_phone = format_phone_for_whatsapp(e_phone)
                    full_wa_msg = create_whatsapp_text(q_no, e_cust_name, e_category, current_e_items, e_subtotal, e_tax_total, e_grand_total)
                    st.link_button("💬 Send Updated via WhatsApp", f"https://wa.me/{wa_eq_phone}?text={urllib.parse.quote(full_wa_msg)}", use_container_width=True)

                q_html_saved = generate_quotation_html(q_no, q_date, e_cust_name, e_phone, e_category, current_e_items, e_subtotal, e_tax_total, e_grand_total)
                components.html(q_html_saved, height=800, scrolling=True)
        else:
            st.info("No saved quotations found.")

    # --- TAB 3: PRODUCT MASTER CATALOG ---
    with tab_catalog:
        st.subheader("📦 Master Product List & Pricing")
        df_master = pd.DataFrame(master_prods, columns=["Item Name", "Unit Price (₹)", "Tax %"])
        st.dataframe(df_master, use_container_width=True)


# ----------------- 3. ANALYTICS & EXCEL REPORTS -----------------
elif menu == "📊 Analytics & Excel Reports":
    st.subheader("📊 Analytics & Export Options")
    
    col_rep1, col_rep2 = st.columns(2)
    
    with col_rep1:
        st.markdown("### 📋 Services Report")
        df_services = pd.DataFrame(all_rows, columns=[
            "Job No", "Customer Name", "Phone", "Item Type", "Item Desc", "Complaint", "Received Date", "Status", "Service Charge (₹)", "Delivery Date"
        ])
        st.dataframe(df_services, use_container_width=True, hide_index=True)
        
        # EXCEL DOWNLOAD BUTTON FOR SERVICES
        buffer_services = io.BytesIO()
        with pd.ExcelWriter(buffer_services, engine='openpyxl') as writer:
            df_services.to_excel(writer, index=False, sheet_name='Services')
        
        st.download_button(
            label="📥 Download Services Data (Excel)",
            data=buffer_services.getvalue(),
            file_name=f"Services_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )

    with col_rep2:
        st.markdown("### 📄 Quotations Report")
        df_quotes = pd.DataFrame(all_quotes, columns=[
            "Quote No", "Customer Name", "Phone", "Category", "Date", "Items JSON", "Subtotal (₹)", "Tax Total (₹)", "Grand Total (₹)"
        ])
        st.dataframe(df_quotes.drop(columns=["Items JSON"]), use_container_width=True, hide_index=True)
        
        # EXCEL DOWNLOAD BUTTON FOR QUOTATIONS
        buffer_quotes = io.BytesIO()
        with pd.ExcelWriter(buffer_quotes, engine='openpyxl') as writer:
            df_quotes.to_excel(writer, index=False, sheet_name='Quotations')
        
        st.download_button(
            label="📥 Download Quotations Data (Excel)",
            data=buffer_quotes.getvalue(),
            file_name=f"Quotations_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )